...
```{r}
library(rio)
databaraa<-import("https://raw.githubusercontent.com/Safwa40/MPDW/main/Data/Data1.csv",header=T)
databara<-datatransformasi
...
Berdasarkan uji ADF tersebut, didapat *p-value* sebesar 0.01 yang lebih besar dari taraf nyata 5% sehingga terima $H_0$ atau data stasioner dalam rataan. Hal ini sesuai dengan hasil eksplorasi menggunakan plot time series dan plot ACF, sehingga dalam hal ini ketidakstasioneran data belum berhasil ditangani dan belum dapat dilanjutkan ke pemodelan
...
Identifikasi model menggunakan plot EACF dilakukan dengan melihat ujung segitiga pada pola segitiga nol. Dalam hal ini model tentatif yang terbentuk adalah ARIMA(0,1,1), ARIMA(0,1,2), ARIMA(0,1,3), ARIMA(1,1,2), ARIMA(1,1,3), ARIMA(2,1,3), dan ARIMA(4,1,1).
...
```{r}
model3.da=Arima(train.diff2, order=c(0,1,3),method="ML")
summary(model3.da) #AIC=507.3
...
```{r}
model4.da<-Arima(train.diff2, order=c(1,1,2),method="ML")
summary(model4.da) #AIC=507.82
...
```{r}
model5.da=Arima(train.diff2, order=c(1,1,3),method="ML")
summary(model5.da) #AIC=509.3 
...
```{r}
model6.da=Arima(train.diff2, order=c(2,1,3),method="ML")
summary(model6.da) #AIC=510.38
lmtest::coeftest(model6.da) # 4/5 parameter signifikan
```
...
### Penanganan ketidaknormalan data
```{r}
library(car)
transformasi <- powerTransform(databara)
transformasi #-1.172155 

datatransformasi <-databara^-1.172155
datatransformasi
```
```{r}

...
```{r}
library(rio)
databara<-import("https://raw.githubusercontent.com/Safwa40/MPDW/main/Data/Data1.csv",header=T)
databara<-databara$`Batubara(USD/ton)`
...
Plot Boxcox menunjukkan nilai *rounded value* ($\lambda$) optimum sebesar **0.3535354** dan pada selang kepercayaan 95% nilai memiliki batas bawah **-0.65656566** dan batas atas **1.36363636**. Selang tersebut memuat nilai satu sehingga dapat dikatakan bahwa data bangkitan stasioner dalam ragam.
...
Berdasarkan uji ADF tersebut, didapat *p-value* sebesar 0.0381 yang lebih kecil dari taraf nyata 5% sehingga tolak $H_0$ atau data stasioner dalam rataan. Hal ini sesuai dengan hasil eksplorasi menggunakan plot time series dan plot ACF, sehingga dalam hal ini ketidakstasioneran data sudah berhasil ditangani dan dapat dilanjutkan ke pemodelan
...
Identifikasi model menggunakan plot EACF dilakukan dengan melihat ujung segitiga pada pola segitiga nol. Dalam hal ini model tentatif yang terbentuk adalah ARIMA(0,1,1), ARIMA(0,1,2), ARIMA(1,1,1),ARIMA(1,1,2), ARIMA(2,1,2), dan ARIMA(0,1,3).
...
```{r}
model4.da<-Arima(train.diff, order=c(0,1,3),method="ML")
summary(model4.da) #AIC=550.52
lmtest::coeftest(model4.da) #seluruhhanya 1 parameter signifikan
```
...
summary(model5.da) #AIC=550.55 
lmtest::coeftest(model5.da) #hanya 1 parameter signifikan
```
...
summary(model6.da) #AIC=551.15
lmtest::coeftest(model6.da) #hanya 2 parameter signifikan
```
...
Berdasarkan uji Ljung-Box tersebut, didapat *p-value* sebesar 0.9849 yang lebih besar dari taraf nyata 5% sehingga tak tolak $H_0$ dan menandakan bahwa sisaan saling bebas. Hal ini berbeda dengan eksplorasi.
...
Berdasarkan uji-ttersebut, didapat *p-value* sebesar 0.8572 yang lebih besar dari taraf nyata 5% sehingga tak tolak $H_0$ dan menandakan bahwa nilai tengah sisaan sama dengan nol. Hal ini sesuai dengan eksplorasi.
...
summary(model1a.ma2) #550.55
lmtest::coeftest(model1a.ma2) #ar1 dan ma2 tidak signifikan

model1b.ma2=Arima(train.diff, order=c(0,1,3),method="ML")
summary(model1b.ma2) #550.52
...
```{r}
pt_1 <- train.ts[96] #nilai akhir data latih

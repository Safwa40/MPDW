...
pacf(sisaan)
```
Didapatkan kesimpulan bahwa data memiliki auto korelasi
...
tabi <- data.frame("rho" = rho, "SSE" = sapply(r, function(i){deviance(hildrethlu.func(i, modelmeteor))}))
round(tabi, 4)
```
...
tabOpt <- data.frame("rho" = rOpt, "SSE" = sapply(rOpt, function(i){deviance(hildreth.lu.func(i, modelmeteor))}))
head(tabOpt[order(tabOpt$SSE),])

#Grafik SSE optimum
par(mfrow = c(1,1))
plot(tabi$SSE ~ tab$rho , type = "l", xlab = "Rho", ylab = "SSE")
...
```{r}
#Model terbaik
modelmeteorHL <- hildreth.lu.func(0.867, modelmeteor)
modelmeteorHL
...
Berdasarkan hasil tersebut dapat diketahui bahwa hasil penanganan autokorelasi dengan metode Cochrane-Orcutt dan Hildreth-Lu memiliki SSE yang sangat berbeda, nilai Model Hildreth-Lu lebih baik dibandingkan model awal ketika autokorelasi masih terjadi, yaitu sebesar $0.3286364$.

# Simpulann
Autokorelasi yang terdapat pada data PS, QV2M, T2M, WS10M, WS10M_RANGE terjadi akibat adanya korelasi di antara unsur penyusunnya. Indikator TS erat hubungannya dengan lingkungan yang sangat rawan menjadi penyebab adanya autokorelasi. Adanya autokorelasi menyebabkan model regresi kurang baik karena akan menigkatkan galatnya. Autokorelasi dapat dideteksi secara eksploratif melalui plot sisaan, ACF, dan PACF, serta dengan uji formal Durbin-Watson. Namun, autokorelasi tersebut dapat ditangani dengan metode Cochrane-Orcutt dan Hildreth-Lu. Dilihat dari nilai SSE, model terbaik ditangani menggunakan Model Hildreth-Lu
